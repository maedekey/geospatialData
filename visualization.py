...
def retrieveInDb(station1, station2, epoch):
    ...
    if validDepartureStation[0][0][-1] == validArrivalStation[0][-1]:
        tripId = validArrivalStation[0][-1]
        print(validDepartureStation)
        print(validArrivalStation)
...
class gtfsData:
    def __init__(self):
        query = ["Ans", "Tirlemont", 1679955725]
        trip = retrieveInDb(query[0], query[1], query[2])
        startTime = trip[0][4]
        endTime = trip[-1][4]
        osm.retrieveTripCoordinates(startTime, trip)
        visualizeTrains(query[2])
